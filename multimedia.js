...
for (var i = 0; i < data.music.length; i++) {
  console.log(
    `Titre ${data.music[i].titre}, Artiste ${data.music[i].artiste}, Studio ${data.music[i].studio}, Language ${data.music[i].language}, Time ${data.music[i].time}, Date release ${data.music[i].dateRelease}, Album ${data.music[i].album}`
  );
}